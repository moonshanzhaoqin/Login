...
public class TransferController {
	...
	public @ResponseEncryptBody TransactionPreviewResponse transactionPreview(
		...
		// 装张金额上限
		if (!reqMsg.getCurrency().equals(ServerConsts.CURRENCY_OF_GOLDPAY) && new Double(reqMsg.getTransAmount()) < 0.0001) {
			...
			return rep;
				...
				&& (new Double(reqMsg.getTransAmount()) % 1 > 0 || new Double(reqMsg.getTransAmount()) == 0)) {
			logger.info("The GDQ must be an integer value");
			rep.setRetCode(RetCodeConsts.TRANSFER_LESS_THAN_MINIMUM_AMOUNT);
			rep.setMessage("The GDQ must be an integer value");
			return rep;
		} else if (new Double(reqMsg.getTransAmount()) > configManager.getConfigLongValue(ConfigKeyEnum.ENTERMAXIMUMAMOUNT,
				1000000000L)) {
		...
		HashMap<String, String> map = transferManager.transactionPreview(sessionData.getUserId(),
				reqMsg.getAreaCode(), reqMsg.getUserPhone(), reqMsg.getCurrency(), new BigDecimal(reqMsg.getTransAmount()));

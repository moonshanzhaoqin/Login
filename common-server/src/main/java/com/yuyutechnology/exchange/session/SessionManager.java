...
public class SessionManager
{
	...
	public void saveSessionData(SessionData sessionData)
	{
		if (sessionData.isLogin()) {
			...
			String key = StringUtils.replace(SESSION_DATA_KEY, "sessionid",
	...
	* @throws
	...
	{
		String json = JsonBinder.getInstance().toJson(sessionData);
		String useridkey = StringUtils.replace(SESSION_DATA_KEY_USERID,
				"userid", sessionData.getUserId().toString());
	...
	public SessionData get(String sessionId)
	{
		...
		{
			return JsonBinder.getInstance().fromJson(jsonContent,
					SessionData.class);
		}
	}
	/**
	...
	{
		String key = StringUtils.replace(SESSION_DATA_KEY_USERID, "userid",
				userId);
		String jsonContent = sessionRedisTemplate.opsForValue().get(key);
		if (StringUtils.isEmpty(jsonContent))
		{

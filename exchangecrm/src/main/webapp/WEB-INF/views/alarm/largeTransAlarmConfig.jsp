...
<html>
	...
	<body>
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-sm-6">
					<h4 class="text-left">大额支付预警信息详情</h4>	
				</div>
				<div class="col-sm-6 text-right" >
					<button class="btn btn-primary " onclick="addLargeTransConfig(1)">添加预警信息</button>
				</div>	
			</div>
					...
					<tbody>
						<c:if test="${not empty list }">
							<c:forEach var="alarmConfig" items="${list}">
								<c:if test="${alarmConfig.alarmType eq 1 }">
									<tr>
			...
			<div class="row">
				<div class="col-sm-6">
					<h4 class="text-left">大额兑换预警信息详情</h4>	
				</div>
				<div class="col-sm-6 text-right" >
					<button class="btn btn-primary " onclick="addLargeTransConfig(2)">添加预警信息</button>
				</div>	
			</div>
			...
			<div class="row">
				<table class = "table table-striped table-bordered">
					...
					<tbody>
						<c:if test="${not empty list }">
							<c:forEach var="alarmConfig" items="${list}">
								<c:if test="${alarmConfig.alarmType eq 2 }">
									<tr>
										<td style="display:none">${alarmConfig.alarmId }</td>
										<td>${alarmConfig.lowerLimit }%</td>
										<td>${alarmConfig.upperLimit }%</td>

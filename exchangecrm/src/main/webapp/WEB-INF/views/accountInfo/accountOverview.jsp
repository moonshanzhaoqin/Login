	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="" style="float: right;">
				<form id="searchForm"
					action="<c:url value='/account/getTotalAssetsInfoByPage' />"
					method="POST">
					<ul class="formbar">
						<li><input type="text" value="${model.userPhone }"
							class="form-control" placeholder="手机号" name="userPhone" size="12">
						</li>
						<li><input type="text" value="${model.userName }"
							class="form-control" placeholder="用户名" name="userName" size="12">
						</li>
						<li><select name="userAvailable" class="form-control">
								...
								<c:if
									test="${(model.userAvailable == '3') || (model.userAvailable != '0' && model.userAvailable != '1')}">
									<option value=3 selected>账号状态</option>
									<option value=0>已冻结</option>
									<option value=1>未冻结</option>
								</c:if>
						</select></li>
						<li><select name="loginAvailable" class="form-control">
								...
								<c:if
									test="${(model.loginAvailable == '3') || (model.loginAvailable != '0' && model.loginAvailable != '1')}">
									<option value=3 selected>登录冻结</option>
									<option value=0>已冻结</option>
									<option value=1>未冻结</option>
								</c:if>
						</select></li>
						<li><select name="payAvailable" class="form-control">
								...
								<c:if
									test="${(model.payAvailable == '3') || (model.payAvailable != '0' && model.payAvailable != '1')}">
						...
						<li>
							<div style="display: none">
								<input name="pageBean.pageSize"
									value="${model.pageBean.pageSize}"> <input
									class="currentPage" id="currentPage"
									name="pageBean.currentPage"
									value="${model.pageBean.currentPage}"> <input
									name="pageBean.pageTotal" value="${model.pageBean.pageTotal}">
							</div>
						</li>
						...
						<li><c:if test="${updateFlag == 0 }">
								<button type="button" class="btn btn-primary"
									id="updateImmediately">立即更新</button>
							</c:if> <c:if test="${updateFlag == 1 }">
					...
					<c:if test="${not empty model.pageBean.rows }">
						<c:forEach var="userInfo" items="${model.pageBean.rows}">
							<tr id=${userInfo.userId }>
								...
								<!-- 									<td></td> -->
								<td><c:if test="${userInfo.userType eq 1}">
										<font color="red">系统用户</font>
									</c:if> <c:if test="${userInfo.userType eq 0}">普通用户</c:if></td>
								<td><c:if test="${userInfo.userAvailable eq 1}">未冻结</c:if>
									<c:if test="${userInfo.userAvailable eq 0}">已冻结</c:if></td>
								<td><c:if test="${userInfo.loginAvailable eq 1}">未冻结</c:if>
									<c:if test="${userInfo.loginAvailable eq 0}">已冻结</c:if></td>
								<td><c:if test="${userInfo.payAvailable eq 1}">未冻结</c:if> <c:if
										test="${userInfo.payAvailable eq 0}">已冻结</c:if></td>
								<td>${userInfo.userTotalAssets }</td>
								<td><fmt:formatDate value="${userInfo.updateAt }"
										pattern="yyyy-MM-dd HH:mm:ss" timeZone="GMT+8" /></td>
								<td><c:if test="${userInfo.userAvailable eq 1}">
										<a href="#" onclick="userFreeze(this,0)">冻结用户</a>
									</c:if> <c:if test="${userInfo.userAvailable eq 0}">
